...
from tensorflow.keras.layers import Input, Conv2D, BatchNormalization, Activation, MaxPool2D, UpSampling2D, Concatenate, Add, Dense
...
def u2net(input_shape, out_ch, int_ch, num_classes=1):
    ...
    y0 = Activation("relu", name="y0")(y0)
    y1 = Activation("relu", name="y1")(y1)
    y2 = Activation("relu", name="y2")(y2)
    y3 = Activation("relu", name="y3")(y3)
    y4 = Activation("relu", name="y4")(y4)
    y5 = Activation("relu", name="y5")(y5)
    y6 = Activation("relu", name="y6")(y6)

    # New Softmax Layer
    new_softmax_layer = Dense(num_classes, activation='relu', name='new_softmax')(y6)
    new_softmax_layer = Activation('relu')(new_softmax_layer)

    model = tf.keras.models.Model(inputs=inputs, outputs=[y0, y1, y2, y3, y4, y5, y6, new_softmax_layer])
    return model
